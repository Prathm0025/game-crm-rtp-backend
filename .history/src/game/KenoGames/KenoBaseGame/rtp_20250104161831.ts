...
export function generatePaytables(
    ...
    let scalingFactor = 1.0; // Adjust this scaling factor to fit the target RTP range

    // Loop through pick sizes and calculate the corresponding RTP
    for (let picks = 1; picks <= maxPicks; picks++) {
        const paytable: { match: number; payout: number; probability: number }[] = [];
        let totalExpectedReturn = 0;

        // Generate paytable for this number of picks
        for (let match = 0; match <= picks; match++) {
            const probability = hypergeometric(N, picks, n, match);
            const payout = payoutMultiplier(match, picks, scalingFactor); // Use the multiplier-based payouts

            totalExpectedReturn += probability * payout;
            paytable.push({ match, payout, probability });
        }

        // Calculate and scale the RTP for this pick size to match the target RTP
        const currentRTP = totalExpectedReturn * 100;
        const adjustmentFactor = targetRTP / currentRTP; // Adjust the scaling factor to get the target RTP

        scalingFactor *= adjustmentFactor; // Adjust the scaling factor for subsequent picks
        rtps[picks] = currentRTP; // Store the current RTP for this pick size
...
// Example payout multiplier function (modified to include scalingFactor)
export function examplePayoutMultiplier(match: number, picks: number, scalingFactor: number): number {
    ...
    // Scale the multiplier to fit within the target RTP
    const adjustedMultiplier = match === 0 ? 0 : baseMultiplier  / (picks / 2);
...
const targetRTP = 76; // Target RTP for all pick sizes (can be between 76% and 77%)

const { paytables, rtps, overallRTP } = generatePaytables(N, n, maxPicks, targetRTP, examplePayoutMultiplier);
