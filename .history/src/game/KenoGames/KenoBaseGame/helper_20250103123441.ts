...
import { bbsRNG } from "./BlumBlumShum";
...
export function makeResultJson(gameInstance: KenoBaseGame) {
  try {
    ...
    const p = 11; // First prime number (congruent to 3 mod 4)
    const q = 19; // Second prime number (congruent to 3 mod 4)
const seed = 7; // Optional seed
