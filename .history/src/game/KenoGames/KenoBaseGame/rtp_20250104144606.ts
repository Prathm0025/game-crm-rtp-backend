...
export function examplePayoutMultiplier(match: number, picks: number): number {
    const baseMultiplier = Math.pow(2, match); // Increase multiplier based on matches
    const uniformScalingFactor = 0.9; // To scale payouts appropriately across pick sizes

    // If we're dealing with lower picks, apply a smaller scaling factor
    const adjustedMultiplier = match === 0 ? 0 : Math.floor(baseMultiplier * uniformScalingFactor / (picks / 2));

    // Ensure that the multiplier is never less than 1 for positive payouts
    return Math.max(adjustedMultiplier, 1);
}
