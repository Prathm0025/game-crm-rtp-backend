...
export function examplePayoutMultiplier(match: number, picks: number): number {
     // Define a base payout that increases gradually with more picks
     const baseMultiplier = 1 + (match * 0.5); // Slowly increase payout for each match
    
     // Apply a scaling factor to reduce the effect of the number of picks on the multiplier
     const scalingFactor = 1 + (picks / 5); // Adjust for larger number of picks without causing major jumps in RTP
     
     // Multiply the base multiplier by the scaling factor and round it to ensure it's an integer
     const adjustedMultiplier = match === 0 ? 0 : Math.floor(baseMultiplier * scalingFactor);
