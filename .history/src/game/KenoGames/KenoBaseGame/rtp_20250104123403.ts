...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    if (match === picks) return 5000; // Jackpot for all matches

    // Intermediate payouts
    const baseMultiplier = Math.pow(2, match);
    const scaledMultiplier = match < picks / 2 ? baseMultiplier / 2 : baseMultiplier;
