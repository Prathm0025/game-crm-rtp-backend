...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    if (match === picks) return 500; // Reduced jackpot for all matches

    // Intermediate payouts
    const baseMultiplier = Math.pow(1.5, match); // Lower base multiplier
    const scaledMultiplier = match < picks / 2 ? baseMultiplier / 3 : baseMultiplier / 2;
