...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    const scalingFactor = 0.7; // To keep the RTP in a balanced range across different pick sizes
    
    // Scale the multiplier slightly based on the number of picks to maintain similar RTPs
    const adjustedMultiplier = match === 0 ? 0 :Math.floor(baseMultiplier * scalingFactor / (picks / 2));
    return Number(adjustedMultiplier.toFixed(1));
}
