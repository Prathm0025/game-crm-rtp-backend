...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    const uniformScalingFactor = 1.5; // Adjust this factor to ensure similar RTPs for all pick sizes

    // Scale the multiplier by a uniform factor that keeps RTPs close across different pick sizes
    const adjustedMultiplier = match === 0 ? 0 : Math.floor(baseMultiplier * uniformScalingFactor);
