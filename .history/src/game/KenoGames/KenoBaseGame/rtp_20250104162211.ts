...
// Function to generate paytables and calculate RTPs for each pick size, adjusted for the target RTP
export function generatePaytables(
    ...
    for (let picks = 1; picks <= maxPicks; picks++) {
        ...
        // Calculate the scaling factor required to adjust RTP to the desired value
        const currentRTP = totalExpectedReturn * 100;
        const adjustmentFactor = desiredRTP / currentRTP; // Adjust the scaling factor to get the target RTP

        scalingFactor *= adjustmentFactor; // Apply the adjustment to scaling factor for subsequent picks
...
const desiredRTP = 90; // Desired RTP for all pick sizes (can be any value between 86 and 100%)
