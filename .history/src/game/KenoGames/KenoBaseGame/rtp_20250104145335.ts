...
export function generatePaytables(
    ...
    // Desired uniform RTP range (93% - 94%)
    const targetMinRTP = 93;
    const targetMaxRTP = 94;
...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    const scalingFactor = 1.2; // Slightly adjusted scaling factor to keep payouts reasonable
