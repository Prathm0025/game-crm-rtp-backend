...
// Adapter for BBSRandomGenerator to fit the RNG interface
class BBSRNGAdapter implements RNG {
  private generator: BBSRandomGenerator;

  constructor(p: number, q: number, seed?: number) {
      this.generator = new BBSRandomGenerator(p, q, seed);
  }

  next(): number {
      // Use generateNumber for a specific range or convert generated bits to a number
      return this.generator.generateNumber(0, 100); // Example range: [0, 100]
  }
}

// Example primes and seed
const p = 11; // Replace with a larger prime congruent to 3 mod 4
const q = 19; // Replace with a larger prime congruent to 3 mod 4
const seed = 7;

// Add BBS RNG to rngs array
const rngs: { name: string; rng: RNG }[] = [
  { name: 'BSS', rng: new BBSRNGAdapter(p, q, seed) },
];
