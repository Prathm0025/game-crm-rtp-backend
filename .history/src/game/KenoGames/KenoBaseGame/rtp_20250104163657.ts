...
export function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    const scalingFactor = 1.5; // For example, testing with scaling factor 1.5
    const estimatedRTP = estimateRTP(scalingFactor);
...
export function estimateRTP(scalingFactor: number): number {
    ...
    // Apply proportional relationship to estimate RTP for any scaling factor
    const estimatedRTP = (scalingFactor / referenceScalingFactor) * referenceRTP;

    // Make sure the RTP stays within a reasonable range (e.g., between 0% and 100%)
    return Math.max(0, Math.min(100, estimatedRTP));
}
