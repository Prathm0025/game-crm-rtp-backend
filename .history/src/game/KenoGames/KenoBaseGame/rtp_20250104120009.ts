...
function generatePaytables(N: number, n: number, maxPicks: number, payoutMultiplier: (k: number, picks: number) => number) {
...
function examplePayoutMultiplier(match: number, picks: number): number {
    ...
    if (match === picks) return 10000; // Jackpot for matching all picks
    return match * (picks * 2); // Example scaling formula
}
