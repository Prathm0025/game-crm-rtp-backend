...
function initializeGameSettings(gameData, gameInstance) {
    return {
        ...
        freeSpin: {
            ...
            countIncrement: gameData.gameSettings.Symbols.find((s) => s.Name === types_1.specialIcons.freeSpin)
                .multiplier.flat()
                .filter((c) => c !== 0),
        },
        gamble: {
...
function checkLineSymbols(firstSymbol, line, gameInstance, direction = "LTR") {
    try {
        const { settings } = gameInstance;
        const wildSymbol = settings.blueWild.SymbolID || settings.goldWild.SymbolID || "";
        let matchCount = 1;
...
function handleFreeSpin(gameInstance) {
    try {
        const goldRowCount = getRandomFromProbability(gameInstance.settings.freeSpin.goldRowCountProb) - 1;
        let goldRows = [];
        for (let i = 0; i < goldRowCount; i++) {
            goldRows.push(getRandomFromProbability(gameInstance.settings.freeSpin.goldRowsProb) -
                1);
        }
...
function checkForFreeSpin(gameInstance) {
    ...
    try {
        ...
        if (count > 2) {
            if (count === 3)
                count = gameInstance.settings.freeSpin.countIncrement[2];
            else if (count === 4)
                count = gameInstance.settings.freeSpin.countIncrement[1];
            else
                count = gameInstance.settings.freeSpin.countIncrement[0];
        }
        return {
            found: count > 2,
            count: count,
