...
exports.generateInitialHeisenberg = generateInitialHeisenberg;
exports.sendInitData = sendInitData;
exports.replaceCoinsWithValues = replaceCoinsWithValues;
exports.handleCashCollectandLink = handleCashCollectandLink;
...
function initializeGameSettings(gameData, gameInstance) {
    const getSymbolIdByName = (name) => {
        const symbol = gameData.gameSettings.Symbols.find((s) => s.Name === name);
...
function generateInitialReel(gameSettings) {
    const reels = [[], [], [], [], []];
    const validSymbols = gameSettings.Symbols.filter(symbol => !symbol.useHeisenberg || symbol.Name === "CashCollect");
...
function generateInitialHeisenberg(gameSettings) {
    const reels = [[], [], [], [], []];
...
function findIndicesOfSymbol(symbolId, matrix) {
    try {
        const indices = [];
...
function checkLineSymbols(firstSymbol, line, gameInstance) {
    try {
        ...
        const getSymbolSettings = (symbolId) => {
            return settings.Symbols.find(symbol => symbol.Id === parseInt(symbolId));
        };
        const isWildSubAllowed = (symbolId) => {
...
function getRandomCoinValue(gameInstance) {
...
function replaceCoinsWithValues(gameInstance, matrixType) {
    const { settings } = gameInstance;
...
function getCoinValue(coinArray) {
    return coinArray.reduce((total, coin) => total + coin.value, 0);
}
function generateHeisenbergSpin(gameInstance) {
    const { settings } = gameInstance;
    const heisenbergReels = [[], [], [], [], []];
    const heisenbergSymbols = settings.Symbols.filter((symbol) => symbol.useHeisenberg);
...
function checkHeisenbergJackPot(gameInstance) {
    const { settings } = gameInstance;
...
function handleCashCollectandLink(gameInstance) {
...
function handleHeisenbergSpin(gameInstance) {
    const { settings, currentGameData } = gameInstance;
...
function handleCashCollectandLospollos(gameInstance) {
    const { settings } = gameInstance;
...
function handleCashCollectandPrizeCoin(gameInstance) {
    const { settings } = gameInstance;
...
function checkForWin(gameInstance) {
    ...
    try {
        ...
        for (let lineIndex = 0; lineIndex < linesApiData.length; lineIndex++) {
            ...
            const firstSymbolId = (_a = resultSymbolMatrix[line[0]]) === null || _a === void 0 ? void 0 : _a[0];
        ...
        if ((hasCashCollect && (hasLinkSymbols || hasMegaLinkSymbols)) && !settings.heisenberg.isTriggered) {
            ...
            settings.heisenbergFreeze = new Set();
        ...
        const sendData = {
            GameData: {
                ...
                bonus: {
                    ...
                    freezeIndices: Array.from(settings.heisenbergFreeze, item => item.split(',').map(Number)),
                },
            },
            PlayerData: {
                Balance: Balance,
